...
def floats_to_colors(values, mode='colormap', range_start=None, range_end=None, colormap='viridis'):
    ...
    Args:
        ...
        mode (str, optional): Color mapping mode (see float_to_color docstring). Defaults to 'colormap'.
        range_start (float, optional): The minimum value for normalization. If None, calculated from the input data.
        range_end (float, optional): The maximum value for normalization. If None, calculated from the input data.
        colormap (str or Colormap, optional): The colormap to use in 'colormap' mode. Defaults to 'viridis'.
    ...
    def _normalize(value):  
        if range_start == range_end:  # Handle potential division by zero
            return 0.5  # Return a neutral value if the input range is degenerate 
        return (value - range_start) / (range_end - range_start)

    normalized_values = [_normalize(val) for val in values]

    colors = []
    if mode == 'hsv':
        for val in normalized_values:
            colors.append(mcolors.hsv_to_rgb((val, 1.0, 1.0)))  # Hue mapping
        ...
        raise ValueError("Invalid color mapping mode")
